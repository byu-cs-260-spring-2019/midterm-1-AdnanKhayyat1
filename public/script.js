var app = new Vue({
    ...
    methods: {
        async bookFind() {
            try{
                ...
                for(var i = 0; i < data.docs.length; i++){
                    ...
                    this.myBooks.push({title : data.docs[i].title_suggest, author : data.docs[i].author_name, publishDate : data.docs[i].first_publish_year, isbn: data.docs[i].isbn, thumbnail : '', isFave : false});
        ...
        FaveItem(item){
            ...
            this.myBooks[index].isFave = true;

        },
        deleteItem(item){
            var index = this.myBooks.indexOf(item);
            this.myBooks[index].isFave = false;
        }
